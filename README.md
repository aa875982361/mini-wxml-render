...
### 小程序官方demo小程序
使用开发者工具打开小程序：`miniprogram_demo`
如果组件有修改，可以使用以下命令重新编译：
```javascript
npm run build:mini
```
演示：
![官方小程序示例demo动态页面简单演示](./docs/miniprogram_demo_simple.gif)
...
1. 将多个页面编译到同一个页面，意味着用一个页面可以渲染多个页面
2. 将多个页面的逻辑放到云静态资源，前端动态加载页面逻辑。
3. 将页面逻辑放到服务端，承载页面只做渲染，类似于小程序的页面热更新
...
- [ ] wxFor节点紧跟着的节点处理
- [x] 支持其他标签，比如说template
- [x] 支持解析表达式为对象的语法，比如说：data="{{title: "xxxx"}}"
- [x] 编译小程序示例demo的文档
- [ ] 支持15层以上的节点
- [ ] 支持编译组件
