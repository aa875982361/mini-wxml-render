# mini-wxml-render
# 高性能动态渲染页面方案
`mini-wxml-render` 可以让你的小程序页面wxml，和你的页面js逻辑整合到一起，通过另外一个页面渲染，在可以动态加载js的前提下，就可以实现小程序页面的热更新。当前微信小程序下是不允许动态加载js的。如果不能动态加载js，本项目也可以作为转化h5项目的一部分。只需要能实现一份浏览器根据虚拟dom树渲染页面的逻辑即可。
